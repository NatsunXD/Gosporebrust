"""Build and inspect the GoPredator v1.0 branch package."""
import json
import os
...
def main():
    ...
    files = {f'data/{ARCHIVE}{suffix}': f'build/{REVISION}/data/{ARCHIVE}{suffix}'
             for suffix in ('', '.stream', '.gpu_resources')}
    report = {
        'name': 'GoPredator', 'slug': 'GoPredator', 'version': '1.0', 'guid': GUID,
        'revision': REVISION,
        'description': 'Adds Predator Variant definitions 1243 and 1245 only to Terminid missions on planet 125, unlocks planet selection, and prepares local 125 task rows from neutral planet records. Mutually exclusive with Gosporebrust. Requires Bingus Shared Loader v15 or newer.',

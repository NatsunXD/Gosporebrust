return function()
    ...
    function api.writable_data(address, size)
        ...
        local region = ffi.new('GosporebrustMemoryRegion[1]')
        while remaining > 0 do
            if query_region(cursor, region, ffi.sizeof(region[0])) ~= ffi.sizeof(region[0]) then
                return false
            end
            -- Only committed private PAGE_READWRITE data is an accepted target.
            if region[0].state ~= 0x1000 or region[0].type ~= 0x20000 or region[0].protection ~= 4 then
                return false
            end

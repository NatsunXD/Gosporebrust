...
- 两个标签均只写入 125 的终结族星球作用域行。
- 重复更新不会重复添加标签。
- 127 与 268 的标签、阵营和任务记录不被修改。
- 125 的动态阵营与可用字段写入会回读验证。
- 125 可从非 268、无特殊星球标签的中立星球获得任务模板。
- owner 变化、非法定义、只读内存和部分写入失败均安全停止或回滚。
- 最终 ZIP 的 Loader 声明、LuaJIT 字节码、归档边界、清单、哈希和文件白名单正确。

## 实机检查

对最终 `GoPredator-v1.0.zip` 的精确 SHA-256 测试：

1. 干净部署 Loader v15 与 GoPredator，确认 Gosporebrust 已禁用。
2. 进入银河战争地图，确认 125 可以点击。
3. 确认 125 出现任务列表并可选择、进入任务。
4. 在关卡内确认掠食变种敌人实际出现。
5. 返回地图并再次进入 125，确认连续覆写后仍正常且没有重复条目或崩溃。
